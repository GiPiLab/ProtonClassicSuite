...
void ProtonClassicSuiteUnitTests::testCaseForTreeNodes() {
  ...
  QCOMPARE(tree.getNodeName(nodeId),
           QString(QString::number(nodeId) + ". " + tree.getTypeNameAndNodeName(nodeId).first + " " + "TESTNODE"));
  QVERIFY(tree.getListOfCompleteNodeNames().contains(QString::number(nodeId) + ". " +
                                                     tree.getTypeNameAndNodeName(nodeId).first + " " + "TESTNODE"));
  ...
  QCOMPARE(tree.getNodeName(nodeId),
           QString(QString::number(nodeId) + ". " + tree.getTypeNameAndNodeName(nodeId).first + " " + "TESTNODENEW"));
...
void ProtonClassicSuiteUnitTests::testCaseForTreeImportExport() {
  ...
  foreach (unsigned int node, randomNodesSortedDFS) { randomNodesNamesSortedDFS.append(randomTree.getNodeName(node)); }
...
void ProtonClassicSuiteUnitTests::testCaseForTreeToDot() {
  ...
  QString attendedDot =
      ...
      "[label=\"7. Maire adjoint 20_NODE6\"];\n6 [label=\"6. Maire adjoint 100_NODE5\"];\n8 [label=\"8. Maire adjoint "
      "200_NODE7\"];\n\t1--2;\n\t1--3;\n\t2--4;\n\t2--5;\n\t3--7;\n\t4--6;\n\t7--8;\n}\n";
