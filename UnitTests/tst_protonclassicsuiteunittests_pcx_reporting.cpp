...
void ProtonClassicSuiteUnitTests::testCaseForReportingNodes() {
  ...
  for (int i = 0; i < (int)PCx_Reporting::OREDPCR::NONELAST; i++) {
    ...
    reporting.setLeafValues(firstLeafId, MODES::DFRFDIRI::DF, QDate(2015, 01, 01),
                            QMap<PCx_Reporting::OREDPCR, double>{{(PCx_Reporting::OREDPCR)i, 10.0}});
    reporting.setLeafValues(lastLeafId, MODES::DFRFDIRI::DF, QDate(2015, 01, 01),
                            QMap<PCx_Reporting::OREDPCR, double>{{(PCx_Reporting::OREDPCR)i, 20.0}});
    QCOMPARE(reporting.getNodeValue(firstLeafId, MODES::DFRFDIRI::DF, (PCx_Reporting::OREDPCR)i, QDate(2015, 01, 01)),
             NUMBERSFORMAT::doubleToFixedPoint(10.0));
    QCOMPARE(reporting.getNodeValue(lastLeafId, MODES::DFRFDIRI::DF, (PCx_Reporting::OREDPCR)i, QDate(2015, 01, 01)),
             NUMBERSFORMAT::doubleToFixedPoint(20.0));
  }
  ...
  for (int i = 0; i < (int)PCx_Reporting::OREDPCR::NONELAST; i++) {
    ...
    QCOMPARE(reporting.getNodeValue(1, MODES::DFRFDIRI::DF, (PCx_Reporting::OREDPCR)i, QDate(2015, 01, 01)),
             NUMBERSFORMAT::doubleToFixedPoint(30.0));
  }
