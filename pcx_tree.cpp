...
unsigned int PCx_Tree::addNode(unsigned int pid, unsigned int typeId, const QString &name, unsigned int forcedNodeId) {
  if (pid == 0 || typeId == 0 || name.isEmpty() || name.size() > MAXNODENAMELENGTH || getNumberOfNodes() > MAXNODES) {
    qFatal("Assertion failed");
  }
  ...
  if (forcedNodeId == 0) {
    ...
    q.prepare(QString("insert into arbre_%1 (id,nom,pid,type) values "
                      "(:id,:nom, :pid, :type)")
                  .arg(treeId));
...
bool PCx_Tree::updateNode(unsigned int nodeId, const QString &newName, unsigned int newType) {
  if (newName.isEmpty() || nodeId == 0 || newName.size() > MAXNODENAMELENGTH || !isTypeIdValid(newType)) {
    qFatal("Assertion failed");
  }
...
QStringList PCx_Tree::getListOfCompleteNodeNames() const {
  ...
  while (q.next()) {
    nodeNames.append(QString("%3. %1 %2")
                         .arg(idTypeToName(q.value("type").toUInt()), q.value("nom").toString())
...
int PCx_Tree::guessHierarchy() {
  ...
  for (QHash<QString, unsigned int>::const_iterator it = prefixToId.constBegin(), end = prefixToId.constEnd();
       it != end; ++it) {
  ...
  for (QHash<unsigned int, unsigned int>::const_iterator it = idToPid.constBegin(), end = idToPid.constEnd(); it != end;
       ++it) {
...
QList<unsigned int> PCx_Tree::sortNodesDFS(QList<unsigned int> &nodes, unsigned int currentNode) const {
  ...
  foreach (unsigned int childId, children) { sortedNodes.append(sortNodesDFS(nodes, childId)); }
  return sortedNodes;
}
...
bool PCx_Tree::deleteNode(unsigned int nodeId) {
  ...
  foreach (unsigned int child, listOfChildrens) { deleteNode(child); }
  deleteNode(nodeId);
...
int PCx_Tree::_internalAddTree(const QString &name, bool createRoot) {
  ...
  if (!query.exec(QString("create table arbre_%1(id integer primary key autoincrement, "
                          ...
                          " type integer references types_%1(id) on delete restrict)")
                      .arg(treeId))) {
    qCritical() << query.lastError();
    die();
  }
  ...
  if (!query.exec(QString("create table types_%1(id integer primary key "
                          "autoincrement, nom text unique not null)")
                      .arg(treeId))) {
    qCritical() << query.lastError();
    die();
  }
...
bool PCx_Tree::validateType(const QString &newType) {
  ...
  if (listOfTypeNames.contains(newType)) {
    QMessageBox::warning(nullptr, QObject::tr("Attention"),
                         QObject::tr("Le type <b>%1</b> existe déjà !").arg(newType.toHtmlEscaped()));
    return false;
  }
  return true;
}
...
bool PCx_Tree::deleteType(unsigned int typeId) {
  ...
  if (query.next()) {
    // qDebug()<<"Number of nodes of type "<<typeId<<" =
    // "<<query.value(0).toInt();
    if (query.value(0).toInt() > 0) {
      QMessageBox::warning(nullptr, QObject::tr("Attention"),
                           QObject::tr("Il existe des noeuds de ce type dans l'arbre. "
                                       "Supprimez-les d'abord, ou changez leur type."));
      return false;
    }
...
QStringList PCx_Tree::getListOfDefaultTypes() {
  ...
  listOfTypes << QObject::tr("Maire adjoint") << QObject::tr("Conseiller") << QObject::tr("Division")
              << QObject::tr("Service");
  return listOfTypes;
}

int PCx_Tree::createRandomTree(const QString &name, unsigned int nbNodes) {
  if (name.isEmpty() || nbNodes == 0 || nbNodes > PCx_Tree::MAXNODES || name.size() > MAXOBJECTNAMELENGTH ||
      treeNameExists(name)) {
    qFatal("Assertion failed");
  }
...
int PCx_Tree::importTreeFromXLSX(const QString &fileName, const QString &treeName) {
  ...
  do {
    ...
    if (!(cellNodeId.isValid() && cellNodeType.isValid() && cellNodeName.isValid() && cellParentId.isValid())) {
      QMessageBox::critical(nullptr, QObject::tr("Erreur"),
                            QObject::tr("Format de fichier invalide ligne %1. Le fichier doit "
                                        "contenir des données sur les quatre premières colonnes")
                                .arg(i));
      return -1;
    }
    ...
    if (aNode.id <= 1) {
      QMessageBox::critical(nullptr, QObject::tr("Erreur"),
                            QObject::tr("Format de fichier invalide ligne %1. L'identifiant du "
                                        "noeud doit être un entier strictement supérieur à 1")
                                .arg(i));
      return -1;
    }
    if (aNode.pid < 1) {
      QMessageBox::critical(nullptr, QObject::tr("Erreur"),
                            QObject::tr("Format de fichier invalide ligne %1. L'identifiant du "
                                        "noeud doit être un entier supérieur ou égal à 1 (1 "
                                        "indique la racine de l'arbre)")
                                .arg(i));
      return -1;
    }
    ...
    if (aNode.typeName.size() > MAXNODENAMELENGTH) {
      QMessageBox::critical(nullptr, QObject::tr("Erreur"),
                            QObject::tr("Type du noeud trop long ligne %1 colonne 2 !").arg(i));
      return -1;
    }

    if (aNode.typeName.isEmpty()) {
      QMessageBox::critical(nullptr, QObject::tr("Erreur"),
                            QObject::tr("Type du noeud manquant ou invalide ligne %1 colonne 2 !").arg(i));
      return -1;
    }

    if (aNode.nodeName.size() > MAXNODENAMELENGTH) {
      QMessageBox::critical(nullptr, QObject::tr("Erreur"),
                            QObject::tr("Nom du noeud trop long ligne %1 colonne 3 !").arg(i));
      return -1;
    }

    if (aNode.nodeName.isEmpty()) {
      QMessageBox::critical(nullptr, QObject::tr("Erreur"),
                            QObject::tr("Nom du noeud manquant ou invalide ligne %1 colonne 3 !").arg(i));
      return -1;
    }
    ...
    if (foundIds.contains(aNode.id)) {
      QMessageBox::critical(nullptr, QObject::tr("Erreur"),
                            QObject::tr("Doublon trouvé ligne %1 colonne 1, il existe déjà un "
                                        "noeud ayant cet identifiant !")
                                .arg(i));
      return -1;
    }
    ...
    if (i > PCx_Tree::MAXNODES) {
      QMessageBox::critical(nullptr, QObject::tr("Erreur"),
                            QObject::tr("Trop de noeuds dans l'arbre (maximum %1) !").arg(PCx_Tree::MAXNODES));
      return -1;
    }
  ...
  foreach (const _node &aNode, foundNodes) {
    if (aNode.pid != 1 && !foundIds.contains(aNode.pid)) {
      QMessageBox::critical(nullptr, QObject::tr("Erreur"),
                            QObject::tr("Le noeud père (%1) du noeud %2 n'existe pas !").arg(aNode.pid).arg(aNode.id));
      return -1;
    }
  }

  // Check if there is at least one first level node, ie one node has the root
  // for PID
  bool okRoot = false;
  foreach (const _node &aNode, foundNodes) {
    if (aNode.pid == 1) {
  ...
  if (!okRoot) {
    QMessageBox::critical(nullptr, QObject::tr("Erreur"),
                          QObject::tr("Aucun noeud n'est accroché à la racine. Renseignez '1' "
                                      "comme identifiant du père colonne 4 pour indiquer un "
                                      "noeud accroché à la racine."));
    return -1;
  }
  ...
  foreach (const QString &oneType, foundTypesSorted) {
    ...
    if (oneTypeId == 0) {
      QMessageBox::warning(nullptr, QObject::tr("Erreur"),
                           QObject::tr("Type %1 invalide").arg(oneType.toHtmlEscaped()));
  ...
  /*
    ...
    if(node1Pid!=1)
    {
        ...
        {
            QMessageBox::critical(0,QObject::tr("Erreur"),QObject::tr("Type ou
nom du noeud parent manquants ligne %1 colonne 4 ou 5 !").arg(i)); return -1;
        }
        if(nodeToPid.contains(node1))
            {
                if(nodeToPid.value(node1)!=node1Pid)
                {
                    QMessageBox::critical(0,QObject::tr("Erreur"),QObject::tr("Le
noeud %1 ne peut pas avoir plusieurs pères !") .arg(QString(node1.first+"
"+node1.second).toHtmlEscaped())); return -1;
                }
                else
                {
                    duplicateFound=true;
        ...
        else
        {
            ...
            {
                QMessageBox::critical(nullptr,QObject::tr("Erreur"),QObject::tr("Type
du noeud père trop long ligne %1 colonne 3 !").arg(i)); return -1;
            }


            if(pidNode.first.isEmpty())
            {
                QMessageBox::critical(0,QObject::tr("Erreur"),QObject::tr("Type
du noeud père manquant ligne %1 colonne 3 !").arg(i)); return -1;
            }

            if(pidNode.second.size()>MAXNODENAMELENGTH)
            {
                QMessageBox::critical(nullptr,QObject::tr("Erreur"),QObject::tr("Nom
du noeud père trop long ligne %1 colonne 4 !").arg(i)); return -1;
            }


            if(pidNode.second.isEmpty())
            {
                QMessageBox::critical(0,QObject::tr("Erreur"),QObject::tr("Nom
du noeud père manquant ligne %1 colonne 4 !").arg(i)); return -1;
            }
        }
    }
    else
    ...
    if(i>PCx_Tree::MAXNODES)
    {
        QMessageBox::critical(0,QObject::tr("Erreur"),QObject::tr("Trop de
noeuds dans l'arbre (%1) !").arg(PCx_Tree::MAXNODES)); return -1;
    }
...
{
    QMessageBox::information(0,QObject::tr("Information"),QObject::tr("Il y a
des doublons dans le fichier, ils ne seront pas pris en compte."));
}


if(firstLevelNodesSet.size()==0)
{
    QMessageBox::critical(0,QObject::tr("Erreur"),QObject::tr("Aucun noeud
n'est accroché à la racine. Laissez les colonnes 3 et 4 vides pour les
spécifier.")); return -1;
}
...
foreach(firstLevelNode,firstLevelNodesSet)
{
    ...
    {
        QMessageBox::critical(0,QObject::tr("Erreur"),QObject::tr("Le noeud %1
ne peut pas avoir plusieurs pères !") .arg(QString(firstLevelNode.first+"
"+firstLevelNode.second).toHtmlEscaped())); return -1;
    }
}
...
{
    if(!nodeToPid.contains(ancestor) &&
!firstLevelNodesSet.contains(ancestor))
    {
        QMessageBox::critical(0,QObject::tr("Erreur"),QObject::tr("Le noeud %1
est orphelin !") .arg(QString(ancestor.first+"
"+ancestor.second).toHtmlEscaped())); return -1;
    }
}
...
foreach(const QString & oneType,foundTypesList)
{
    ...
    if(oneTypeId==0)
    {
        QMessageBox::warning(0,QObject::tr("Erreur"),QObject::tr("Type %1
invalide").arg(oneType.toHtmlEscaped())); QSqlDatabase::database().rollback();
        return -1;
    }
}
