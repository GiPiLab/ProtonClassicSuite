...
TreeMap::TreeMap(TreeMap *parent, QString name, unsigned int id, int year, double value)
    : parent(parent), name(std::move(name)), id(id), year(year), value(value) {}
...
void TreeMap::layout(QList<TreeMap *> items, int start, int end, const QRect &bounds) {
  ...
  if (w < h) {
    ...
    slicelayout(items, start, mid,
                QRect(static_cast<int>(x), static_cast<int>(y), static_cast<int>(w), static_cast<int>(h * b)));
    layout(items, mid + 1, end,
           QRect(static_cast<int>(x), static_cast<int>(y + h * b), static_cast<int>(w), static_cast<int>(h * (1 - b))));
    ...
    slicelayout(items, start, mid,
                QRect(static_cast<int>(x), static_cast<int>(y), static_cast<int>(w * b), static_cast<int>(h)));
    layout(items, mid + 1, end,
           QRect(static_cast<int>(x + w * b), static_cast<int>(y), static_cast<int>(w * (1 - b)), static_cast<int>(h)));
  }
}
...
double TreeMap::aspect(double _big, double _small, double a, double b) const { return (_big * b) / (_small * a / b); }
...
void TreeMap::slicelayout(QList<TreeMap *> items, int start, int end, const QRect &bounds) {
  ...
  for (int i = start; i <= end && i < items.count(); i++) {
    ...
    if (orientation == Qt::Vertical) {
      ...
      items[i]->rect.setY(1 + qRound(static_cast<double>(bounds.y()) +
                                     static_cast<double>(bounds.height()) * (1.0 - accumulator - factor)));
      items[i]->rect.setHeight(qRound(static_cast<double>(bounds.height()) * factor) - 1);
    } else {
      items[i]->rect.setX(1 + qRound(static_cast<double>(bounds.x()) +
                                     static_cast<double>(bounds.width()) * (1.0 - accumulator - factor)));
      items[i]->rect.setWidth(qRound(static_cast<double>(bounds.width()) * factor) - 1);
...
bool TreeMap::lessThan(const TreeMap *a, const TreeMap *b) { return (a->value) > (b->value); }
