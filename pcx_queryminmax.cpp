...
PCx_QueryMinMax::PCx_QueryMinMax(PCx_Audit *model, unsigned int queryId) : PCx_Query(model) { load(queryId); }

PCx_QueryMinMax::PCx_QueryMinMax(PCx_Audit *model, unsigned int typeId, PCx_Audit::ORED ored, MODES::DFRFDIRI dfrfdiri,
                                 qint64 val1, qint64 val2, int year1, int year2, const QString &name)
    : PCx_Query(model, typeId, ored, dfrfdiri, year1, year2, name) {
...
QString PCx_QueryMinMax::exec(QXlsx::Document *xlsDoc) const {
  ...
  if (typeId != ALLTYPES) {
    q.prepare(QString("select id_node,annee,%1 from audit_%2_%3 as a, arbre_%4 "
                      ...
                      "and type=:typeId and %1 not null and %1>=:val1 and "
                      "%1<=:val2 and annee>=:year1 and "
                      "annee<=:year2")
  ...
  output.append(QString("<table class='req3' cellpadding='5' align='center' "
...
bool PCx_QueryMinMax::load(unsigned int queryId) {
  ...
  if (!q.next()) {
    ...
    if (static_cast<PCx_Query::QUERIESTYPES>(q.value("query_mode").toUInt()) != PCx_Query::QUERIESTYPES::MINMAX) {
...
QString PCx_QueryMinMax::getDescription() const {
  ...
  out.append(QObject::tr(" dont les %1 des %2 sont compris entre %3€ et %4€ entre %5 et %6")
                 .arg(PCx_Audit::OREDtoCompleteString(ored, true).toHtmlEscaped(),
                      MODES::modeToCompleteString(dfrfdiri).toLower().toHtmlEscaped(), formatFixedPoint(val1),
                      formatFixedPoint(val2))
                 .arg(year1)
                 .arg(year2));
  return out;
}
