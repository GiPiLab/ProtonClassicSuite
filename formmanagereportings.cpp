...
FormManageReportings::FormManageReportings(QWidget *parent) : QWidget(parent), ui(new Ui::FormManageReportings) {
  ui->setupUi(this);
...
void FormManageReportings::onLOTchanged() { updateListOfTrees(); }

void FormManageReportings::onListOfAuditsChanged() { updateListOfPotentialAudits(); }
...
redo:
  do {
    text = QInputDialog::getText(this, tr("Nouveau reporting"), tr("Nom du reporting à ajouter : "), QLineEdit::Normal,
                                 "", &ok)
               .simplified();
  ...
  if (ok) {
    ...
    if (PCx_Reporting::addNewReporting(text, selectedTree) > 0) {
      QMessageBox::information(this, tr("Information"),
                               tr("Nouveau reporting ajouté, utilisez le bouton <b>générer un "
                                  "fichier squelette</b> afin de créer un fichier avec les feuilles "
                                  "de l'arbre et les colonnes à remplir. Ce fichier doit être "
                                  "dupliqué pour les DF, RF, DI, RI le cas échéant, puis complété "
                                  "sous excel avec vos données. Une fois les fichiers complétés, "
                                  "utilisez les boutons <b>Charger [x]</b> pour les importer dans "
                                  "l'audit. Vous pouvez également remplir le reporting de données "
                                  "aléatoires à des fins de test"));
      updateListOfReportings();
...
void FormManageReportings::updateListOfReportings() {
  ...
  if (!lot.isEmpty() && listOfReportings.isEmpty()) {
    QMessageBox::information(this, tr("Information"),
                             tr("Vous pouvez maintenant créer un nouveau reporting. Une fois créé, "
                                "utilisez le bouton <b>Générer le fichier squelette</b> pour "
                                "fabriquer un ficher excel que vous dupliquerez (un pour les DF, un "
                                "pour les RF, ainsi de suite le cas échéant) et dont vous "
                                "compléterez les colonnes avec vos données, avant de les charger "
                                "avec les boutons <b>Charger [x]</b>. Vous pouvez aussi remplir le "
                                "reporting de données aléatoires à des fins de test."));
  }
...
void FormManageReportings::updateListOfPotentialAudits() {
  ...
  QPair<unsigned int, QString> p;
  foreach (p, listOfAuditsWithThisTree) { ui->comboListOfAudits->insertItem(0, p.second, p.first); }
...
void FormManageReportings::on_pushButtonDeleteReporting_clicked() {
  ...
  if (question(tr("Voulez-vous vraiment <b>supprimer</b> le reporting <b>%1</b> ? "
  ...
  if (selectedReporting != nullptr) {
    ...
    if (res == true) {
      QMessageBox::information(
          this, tr("Succès"),
          tr("%1 chargées !").arg(MODES::modeToCompleteString(MODES::DFRFDIRI::DF).toHtmlEscaped()));
      updateReportingInfos();
    }
  }
}
...
void FormManageReportings::on_pushButtonLoadRF_clicked() {
  if (selectedReporting != nullptr) {
    ...
    if (res == true) {
      QMessageBox::information(
          this, tr("Succès"),
          tr("%1 chargées !").arg(MODES::modeToCompleteString(MODES::DFRFDIRI::RF).toHtmlEscaped()));
      updateReportingInfos();
    }
  }
}
...
void FormManageReportings::on_pushButtonLoadDI_clicked() {
  if (selectedReporting != nullptr) {
    ...
    if (res == true) {
      QMessageBox::information(
          this, tr("Succès"),
          tr("%1 chargées !").arg(MODES::modeToCompleteString(MODES::DFRFDIRI::DI).toHtmlEscaped()));
      updateReportingInfos();
    }
  }
}
...
void FormManageReportings::on_pushButtonLoadRI_clicked() {
  if (selectedReporting != nullptr) {
    ...
    if (res == true) {
      QMessageBox::information(
          this, tr("Succès"),
          tr("%1 chargées !").arg(MODES::modeToCompleteString(MODES::DFRFDIRI::RI).toHtmlEscaped()));
      updateReportingInfos();
    }
  }
}
...
void FormManageReportings::on_pushButtonExportDF_clicked() {
  if (selectedReporting != nullptr) {
    ...
    if (res == true) {
      QMessageBox::information(
          this, tr("Succès"),
          tr("%1 enregistrées !").arg(MODES::modeToCompleteString(MODES::DFRFDIRI::DF).toHtmlEscaped()));
    } else {
      QMessageBox::critical(this, tr("Erreur"), tr("Enregistrement échoué"));
    }
  }
...
void FormManageReportings::on_pushButtonExportRF_clicked() {
  if (selectedReporting != nullptr) {
    ...
    if (res == true) {
      QMessageBox::information(
          this, tr("Succès"),
          tr("%1 enregistrées !").arg(MODES::modeToCompleteString(MODES::DFRFDIRI::RF).toHtmlEscaped()));
    } else {
      QMessageBox::critical(this, tr("Erreur"), tr("Enregistrement échoué"));
    }
  }
...
void FormManageReportings::on_pushButtonExportDI_clicked() {
  if (selectedReporting != nullptr) {
    ...
    if (res == true) {
      QMessageBox::information(
          this, tr("Succès"),
          tr("%1 enregistrées !").arg(MODES::modeToCompleteString(MODES::DFRFDIRI::DI).toHtmlEscaped()));
    } else {
      QMessageBox::critical(this, tr("Erreur"), tr("Enregistrement échoué"));
    }
  }
...
void FormManageReportings::updateReportingInfos() {
  if (selectedReporting != nullptr) {
    ui->labelTreeName->setText(QString("%1 (%2 noeuds)")
                                   .arg(selectedReporting->getAttachedTreeName())
                                   .arg(selectedReporting->getAttachedTree()->getNumberOfNodes()));
...
void FormManageReportings::on_pushButtonExportRI_clicked() {
  if (selectedReporting != nullptr) {
    ...
    if (res == true) {
      QMessageBox::information(
          this, tr("Succès"),
          tr("%1 enregistrées !").arg(MODES::modeToCompleteString(MODES::DFRFDIRI::RI).toHtmlEscaped()));
    } else {
      QMessageBox::critical(this, tr("Erreur"), tr("Enregistrement échoué"));
    }
  }
...
void FormManageReportings::on_pushButtonClearDF_clicked() {
  if (question(tr("Voulez-vous vraiment <b>supprimer</b> toutes les données de %2 "
                  "du reporting <b>%1</b> ? Cette action ne peut être annulée")
                   .arg(ui->comboListOfReportings->currentText().toHtmlEscaped(),
                        MODES::modeToCompleteString(MODES::DFRFDIRI::DF).toHtmlEscaped())) == QMessageBox::Yes) {
...
void FormManageReportings::on_pushButtonClearRF_clicked() {
  if (question(tr("Voulez-vous vraiment <b>supprimer</b> toutes les données de %2 "
                  "du reporting <b>%1</b> ? Cette action ne peut être annulée")
                   .arg(ui->comboListOfReportings->currentText().toHtmlEscaped(),
                        MODES::modeToCompleteString(MODES::DFRFDIRI::RF).toHtmlEscaped())) == QMessageBox::Yes) {
...
void FormManageReportings::on_pushButtonClearDI_clicked() {
  if (question(tr("Voulez-vous vraiment <b>supprimer</b> toutes les données de %2 "
                  "du reporting <b>%1</b> ? Cette action ne peut être annulée")
                   .arg(ui->comboListOfReportings->currentText().toHtmlEscaped(),
                        MODES::modeToCompleteString(MODES::DFRFDIRI::DI).toHtmlEscaped())) == QMessageBox::Yes) {
...
void FormManageReportings::on_pushButtonClearRI_clicked() {
  if (question(tr("Voulez-vous vraiment <b>supprimer</b> toutes les données de %2 "
                  "du reporting <b>%1</b> ? Cette action ne peut être annulée")
                   .arg(ui->comboListOfReportings->currentText().toHtmlEscaped(),
                        MODES::modeToCompleteString(MODES::DFRFDIRI::RI).toHtmlEscaped())) == QMessageBox::Yes) {
...
void FormManageReportings::on_pushButtonRandomDF_clicked() {
  if (question(tr("Ajouter des données aléatoires pour les %2 15 jours après la "
                  "dernière situation du reporting <b>%1</b> ?")
                   .arg(ui->comboListOfReportings->currentText().toHtmlEscaped(),
                        MODES::modeToCompleteString(MODES::DFRFDIRI::DF).toHtmlEscaped())) == QMessageBox::Yes) {
...
void FormManageReportings::on_pushButtonFillAudit_clicked() {
  if (question(tr("Ajouter les données de la dernière situation du reporting "
                  "<b>%1</b> à l'audit <b>%2</b> ?")
                   .arg(ui->comboListOfReportings->currentText().toHtmlEscaped(),
                        ui->comboListOfAudits->currentText().toHtmlEscaped())) == QMessageBox::Yes) {
...
void FormManageReportings::on_pushButtonDeleteLastDF_clicked() {
  if (question(tr("Supprimer les valeurs de la dernière situation pour les %1 ?")
                   .arg(MODES::modeToCompleteString(MODES::DFRFDIRI::DF))) == QMessageBox::Yes) {
...
void FormManageReportings::on_pushButtonDeleteLastRF_clicked() {
  if (question(tr("Supprimer les valeurs de la dernière situation pour les %1 ?")
                   .arg(MODES::modeToCompleteString(MODES::DFRFDIRI::RF))) == QMessageBox::Yes) {
...
void FormManageReportings::on_pushButtonDeleteLastDI_clicked() {
  if (question(tr("Supprimer les valeurs de la dernière situation pour les %1 ?")
                   .arg(MODES::modeToCompleteString(MODES::DFRFDIRI::DI))) == QMessageBox::Yes) {
...
void FormManageReportings::on_pushButtonDeleteLastRI_clicked() {
  if (question(tr("Supprimer les valeurs de la dernière situation pour les %1 ?")
                   .arg(MODES::modeToCompleteString(MODES::DFRFDIRI::RI))) == QMessageBox::Yes) {
...
redo:
  do {
    text = QInputDialog::getText(this, tr("Dupliquer reporting"), tr("Nom du reporting dupliqué"), QLineEdit::Normal,
                                 "", &ok)
               .simplified();
  ...
  if (ok) {
    ...
    unsigned int res = selectedReporting->duplicateReporting(text);
