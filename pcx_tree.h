 ...
 * cannot be modified in the user interface. Only a finished tree is allowed to
 * build an audit.
 *
 * Each node in a tree has a name, an unsigned identifier, a parent identifier
 * and a type identifier. The type identifier is a mapping between an unsigned
 * int and a string stored in table "types_[treeId]". Duplicates are not allowed
 * (same name and same type identifier)
 *
 *
 */
class PCx_Tree {
...
public:
  ...
  /**
   * @brief MAXNODENAMELENGTH is the maximum allowed length of a node or type
   * name
   */
   ...
   *
   * Add a node in the tree. Check for the maximum number of nodes and if a node
  ...
  /**
   * @brief PCx_Tree::deleteNode deletes a node and all its children from the
   * database
  ...
  /**
   * @brief getLeavesId gets the identifier of the nodes that are both leaves
   * and descendants of parentNode
  ...
  /**
   * @brief PCx_Tree::getListOfCompleteNodeNames gets the list of full node
   * names (typeName + " " + nodeName)
   * @return The string list of full node names
   */
  QStringList getListOfCompleteNodeNames() const;

  /**
   * @brief getNodesWithSharedName gets nodes that have the same name but not
   * the same type
   ...
   * @param node the node identifier
   * @return the full name of the node, including its type name, joined with a
   * space
   */
  QString getNodeName(unsigned int node) const;

  /**
   * @brief getTypeNameAndNodeName gets the complete name of a node, type name
   * splitted
   * @param node the node identifier
   * @return a pair of QString with the name of the type and the name of the
   * node
   */
  QPair<QString, QString> getTypeNameAndNodeName(unsigned int node) const;

  /**
   * @brief getNumberOfNodesWithThisType gets the number of nodes that have a
   * specific type
  ...
  /**
   * @brief PCx_Tree::nodeExists checks if a node with given name and type
   * already exists
  ...
  /**
   * @brief getNodeIdFromTypeAndNodeName finds the node identifier from its type
   * name and node name
   *
   * The node name is simplified through QString::simplified() before querying
   * the database
   *
   * @param typeAndNodeName a pair of QString with the name of the type and the
   * name of the node
   ...
   * @return the type name, or an empty QString if the type identifier does not
   * exists in the DB
   */
   ...
   */
  bool isTypeIdValid(unsigned int id) const {
    if (id == 0)
      return true;
    else
      return idTypesToNames.contains(id);
  }
   ...
   * @param typeName the type to find, will be simplified with
   * QString::simplified()
   ...
   * If both COL3 and COL4 are empty, the node described in COL1 and COL2 is a
   * first level node
   *
   * Check for duplicates, orphan nodes, multiple ancestors and empty root and
   * print a warning. Stop after the first warning
   *
   ...
   * @return 1 on success, 0 if an audit is attached to this tree, -1 if the
   ...
   * @param finishedOnly indicates if the list should contains only finished
   * trees
   ...
   * Currently uses the numbers in front of each node and build a hierarchy like
   * 102 is child of 10 is child of 1
   ...
   * name (useful for error checking in data import)
...
protected:
  /**
   * @brief treeId the tree identifier in the database, the table is
   * "arbre_treeId"
   */
  unsigned int treeId;

  /**
   * @brief finished is the tree finished ? A finished tree must be read-only in
   * the UI
   */
  bool finished;
  ...
  /**
   * @brief validateType checks if a type with this name can be added in the
   * database
   * @param newType the type name to check
   * @return true if the type name can be added, false if it already exists or
   * is blank. A warning is displayed in the UI.
   */
  bool validateType(const QString &newType);

  /**
   * @brief listOfTypeNames the list of all type names that can be used in this
   * tree
   */
  ...
  /**
   * @brief loadTypesFromDb initializes idTypeToName of listOfTypeNames from the
   * database
   */
   ...
   *
   * Protected method to creates a tree without sql transaction. Used in addTree
   * and duplicateTree
   * @param name the name of the tree
