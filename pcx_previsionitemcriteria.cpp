...
PCx_PrevisionItemCriteria::PCx_PrevisionItemCriteria() = default;

PCx_PrevisionItemCriteria::PCx_PrevisionItemCriteria(const QString &serializedCriteria) {
  unserialize(serializedCriteria);
}

PCx_PrevisionItemCriteria::PCx_PrevisionItemCriteria(PREVISIONOPERATOR previsionOperator, PCx_Audit::ORED ored,
...
QString PCx_PrevisionItemCriteria::getCriteriaLongDescription() const {
  ...
  case PREVISIONOPERATOR::LASTVALUE:
    ...
    break;
  case PREVISIONOPERATOR::PERCENT:
    output = QObject::tr("%1% de la dernière valeur %2")
                 .arg(NUMBERSFORMAT::formatFixedPoint(previsionOperand),
                      PCx_Audit::OREDtoCompleteString(previsionOredTarget, true));
    break;
  case PREVISIONOPERATOR::FIXEDVALUE:
    output = QObject::tr("Valeur fixe");
    break;
  case PREVISIONOPERATOR::FIXEDVALUEFROMPROPORTIONALREPARTITION:
    output = QObject::tr("Distribution en fonction du poids relatif des %1 de l'année N")
                 .arg(PCx_Audit::OREDtoCompleteString(previsionOredTarget, true));
  }
  return output;
}
...
void PCx_PrevisionItemCriteria::fillComboBoxWithOperators(QComboBox *combo) {
  combo->clear();
  combo->addItem(QObject::tr("Minimum"), static_cast<int>(PREVISIONOPERATOR::MINIMUM));
  combo->addItem(QObject::tr("Maximum"), static_cast<int>(PREVISIONOPERATOR::MAXIMUM));
  combo->addItem(QObject::tr("Moyenne"), static_cast<int>(PREVISIONOPERATOR::AVERAGE));
  combo->addItem(QObject::tr("Tendance linéaire"), static_cast<int>(PREVISIONOPERATOR::REGLIN));
  combo->addItem(QObject::tr("Valeur fixe"), static_cast<int>(PREVISIONOPERATOR::FIXEDVALUE));
  combo->addItem(QObject::tr("Dernière valeur"), static_cast<int>(PREVISIONOPERATOR::LASTVALUE));
  combo->addItem(QObject::tr("Pourcentage"), static_cast<int>(PREVISIONOPERATOR::PERCENT));
}
...
QString PCx_PrevisionItemCriteria::serialize() const {
  ...
  case PREVISIONOPERATOR::FIXEDVALUE:
    ...
    break;
  case PREVISIONOPERATOR::FIXEDVALUEFROMPROPORTIONALREPARTITION:
    output = QString("fixedvaluefromproportionalrepartition,%1,%2")
                 .arg(PCx_Audit::OREDtoTableString(previsionOredTarget))
                 .arg(previsionOperand);
    break;
  }
  return output;
}

qint64 PCx_PrevisionItemCriteria::getLastValueOf(unsigned int auditId, MODES::DFRFDIRI mode,
  ...
  q.prepare(QString("select %1 from audit_%2_%3 where id_node=:id_node order "
                    "by annee desc limit 1")
                .arg(PCx_Audit::OREDtoTableString(previsionOredTarget), MODES::modeToTableString(mode))
                .arg(auditId));
...
qint64 PCx_PrevisionItemCriteria::getPercentOf(unsigned int auditId, MODES::DFRFDIRI mode, unsigned int nodeId) const {
  ...
  perc /= 100.0;
  return static_cast<qint64>(lastValue * perc);
}
...
qint64 PCx_PrevisionItemCriteria::getMinimumOf(unsigned int auditId, MODES::DFRFDIRI mode, unsigned int nodeId) const {
  ...
  q.prepare(QString("select min(%1) from audit_%2_%3 where id_node=:id_node")
                .arg(PCx_Audit::OREDtoTableString(previsionOredTarget), MODES::modeToTableString(mode))
                .arg(auditId));
  q.bindValue(":id_node", nodeId);
  if (!q.exec()) {
...
qint64 PCx_PrevisionItemCriteria::getMaximumOf(unsigned int auditId, MODES::DFRFDIRI mode, unsigned int nodeId) const {
  ...
  QSqlQuery q;
  q.prepare(QString("select max(%1) from audit_%2_%3 where id_node=:id_node")
                .arg(PCx_Audit::OREDtoTableString(previsionOredTarget), MODES::modeToTableString(mode))
                .arg(auditId));
  q.bindValue(":id_node", nodeId);
  if (!q.exec()) {
...
qint64 PCx_PrevisionItemCriteria::getAverageOf(unsigned int auditId, MODES::DFRFDIRI mode, unsigned int nodeId) const {
  ...
  QSqlQuery q;
  q.prepare(QString("select avg(%1) from audit_%2_%3 where id_node=:id_node")
                .arg(PCx_Audit::OREDtoTableString(previsionOredTarget), MODES::modeToTableString(mode))
                .arg(auditId));
  q.bindValue(":id_node", nodeId);
  if (!q.exec()) {
...
qint64 PCx_PrevisionItemCriteria::getReglinOf(unsigned int auditId, MODES::DFRFDIRI mode, unsigned int nodeId) const {
  ...
  QSqlQuery q;
  q.prepare(QString("select annee,%1 from audit_%2_%3 where id_node=:id_node "
                    "and %1 not null order by annee")
                .arg(PCx_Audit::OREDtoTableString(previsionOredTarget), MODES::modeToTableString(mode))
                .arg(auditId));
  q.bindValue(":id_node", nodeId);
  if (!q.exec()) {
