...
PCx_Query::PCx_Query(PCx_Audit *model, unsigned int typeId, PCx_Audit::ORED ored, MODES::DFRFDIRI dfrfdiri, int year1,
                     int year2, QString name)
    : model(model), typeId(typeId), ored(ored), dfrfdiri(dfrfdiri), name(std::move(name)), queryId(0) {
  setYears(year1, year2);
}

void PCx_Query::setYears(int year1, int year2) {
  if (year1 > year2) {
    int yeartmp;
    yeartmp = year1;
    year1 = year2;
  ...
  int firstYear = model->getYears().constFirst();
  int lastYear = model->getYears().constLast();
  if (year1 < firstYear) {
    year1 = firstYear;
  }
...
bool PCx_Query::remove(unsigned int queryId) { return deleteQuery(model->getAuditId(), queryId); }

void PCx_Query::createTableQueries(unsigned int auditId) {
  QSqlQuery q;
  if (!q.exec(QString("create table audit_queries_%1(id integer primary key "
                      "autoincrement, name text not null, "
                      "query_mode integer not null, target_type integer check "
                      "(target_type>=0) not null, "
                      "ored integer not null, dfrfdiri integer not null, oper integer, "
                      "percent_or_point integer, increase_decrease integer,val1 "
                      "integer not null default '0', val2 integer,"
                      "year1 integer not null, year2 integer not null)")
                  .arg(auditId))) {
...
QList<unsigned int> PCx_Query::getListOfQueriesId(unsigned int auditId, QUERIESTYPES type) {
  ...
  if (!query.exec(QString("select id from audit_queries_%1 where query_mode=%2")
                      .arg(auditId)
                      .arg(static_cast<unsigned int>(type)))) {
