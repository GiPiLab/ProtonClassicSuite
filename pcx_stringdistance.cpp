...
PCx_StringDistance::PCx_StringDistance(const QString &str1, const QString &str2, SIMILARITYMETRIC similarityMode)
    : str1(str1), str2(str2), metric(similarityMode) {
...
double PCx_StringDistance::normalizedLevenshteinDistance(const QString &sa, const QString &ta, bool caseInsensitive) {
  ...
  QVarLengthArray<int, 1024> tabDistances((maxSize + 1) * (maxSize + 1));
  ...
  return static_cast<double>(distance) / maxSize;
}
...
double PCx_StringDistance::jaroWinklerDistance(const QString &str1, const QString &str2, bool caseInsensitive) {
  ...
  /* Jaro distance */
  dw = ((static_cast<double>(m) / sl) + (static_cast<double>(m) / al) + (static_cast<double>(m - t) / m)) / 3.0;
