...
PCx_Reporting::PCx_Reporting(unsigned int reportingId, bool _noLoadAttachedTree) : reportingId(reportingId) {
  if (reportingId == 0) {
...
bool PCx_Reporting::setLeafValues(unsigned int leafId, MODES::DFRFDIRI mode, QDate date,
  ...
  for (QMap<PCx_Reporting::OREDPCR, double>::const_iterator it = vals.constBegin(), end = vals.constEnd(); it != end;
       ++it) {
  ...
  q.prepare(QString("insert or ignore into %1 (%2) values (%3)").arg(tableName, reqSet, reqBind));
  ...
  q.prepare(QString("update %1 set disponibles=ouverts-(realises+engages) "
                    "where id_node=:id_node and date=:date")
                .arg(tableName));
  q.bindValue(":id_node", leafId);
  q.bindValue(":date", sSinceEpoch);
  if (!q.exec()) {
...
void PCx_Reporting::updateParent(const QString &tableName, QDate date, unsigned int nodeId) {
  ...
  q.prepare(QString("insert into %1 "
                    "(id_node,date,ouverts,realises,engages,disponibles,bp,reports,"
                    "ocdm,vcdm,budgetvote,vcinterne,rattachenmoins1) "
                    "values(:id_node,:date,:ouverts,:realises,:engages,:disponibles,:"
...
bool PCx_Reporting::importDataFromXLSX(const QString &fileName, MODES::DFRFDIRI mode) {
  ...
  if (!(xlsx.read(1, 1).isValid() && xlsx.read(1, 2).isValid() && xlsx.read(1, 3).isValid() &&
        ...
        xlsx.read(1, 7).isValid() && xlsx.read(1, 8).isValid() && xlsx.read(1, 9).isValid() &&
        xlsx.read(1, 10).isValid() && xlsx.read(1, 11).isValid() && xlsx.read(1, 12).isValid() &&
        xlsx.read(1, 13).isValid() && xlsx.read(1, 14).isValid())) {
    QMessageBox::critical(nullptr, QObject::tr("Erreur"),
                          QObject::tr("Format de fichier invalide. Vous pouvez exporter un fichier "
                                      "squelette dans l'interface de gestion des reportings"));
    return false;
  }
  ...
  do {
    QVariant cellNodeId, nodeType, nodeName, date, ouverts, realises, engages, bp, reports, ocdm, vcdm, budgetvote,
        vcinternes, rattachesnmoins1;
    cellNodeId = xlsx.read(row, 1);
    nodeType = xlsx.read(row, 2);
    nodeName = xlsx.read(row, 3);
    // Do not use read for date, always assume "1900" mode to ensure libreoffice
    // compatibility
    if (xlsx.read(row, 4).isNull()) {
      QMessageBox::critical(nullptr, QObject::tr("Erreur"),
                            QObject::tr("Erreur de format ligne %1, spécifiez une date").arg(row));
      return false;
    }
    ...
    if (!(cellNodeId.isValid() && nodeType.isValid() && nodeName.isValid() && date.isValid())) {
      QMessageBox::critical(nullptr, QObject::tr("Erreur"),
                            QObject::tr("Erreur de format ligne %1, remplissez l'identifiant, le "
                                        "type et le nom du noeud ainsi que la date d'application")
                                .arg(row));
      return false;
    }
    ...
    if (typeAndNode.first.isEmpty() || typeAndNode.second.isEmpty()) {
      QMessageBox::critical(nullptr, QObject::tr("Erreur"),
                            QObject::tr("Erreur de format ligne %1, le type et le nom du noeud "
                                        "ne peuvent pas être vides (ni composés d'espaces)")
                                .arg(row));
      return false;
    }
    ...
    if (!qIsFinite(dblOuv) || !qIsFinite(dblReal) || !qIsFinite(dblEng) || !qIsFinite(dblBP) ||
        !qIsFinite(dblReports) || !qIsFinite(dblOCDM) || !qIsFinite(dblVCDM) || !qIsFinite(dblBudgetVote) ||
        !qIsFinite(dblVInternes) || !qIsFinite(dblRattachesNMoins1)) {
      QMessageBox::critical(nullptr, QObject::tr("Erreur"),
                            QObject::tr("Valeur infinie ligne %1 non autorisée !").arg(row));
      return false;
    }

    if (dblOuv < 0.0 || dblReal < 0.0 || dblEng < 0.0 || dblBP < 0.0 || dblReports < 0.0 || dblBudgetVote < 0.0 ||
        dblRattachesNMoins1 < 0.0) {
      QMessageBox::critical(nullptr, QObject::tr("Erreur"),
...
QDate PCx_Reporting::getLastReportingDate(MODES::DFRFDIRI mode, unsigned int node) const {
  QSqlQuery q;
  q.prepare(QString("select date from reporting_%1_%2 where id_node=:idnode "
...
bool PCx_Reporting::addLastReportingDateToExistingAudit(PCx_Audit *audit) const {
  ...
  if (audit->getAttachedTreeId() != attachedTreeId) {
    QMessageBox::warning(nullptr, QObject::tr("Attention"),
                         QObject::tr("L'audit et le reporting ne portent pas sur le même arbre !"));
    return false;
  }
  if (audit->isFinished()) {
    QMessageBox::warning(nullptr, QObject::tr("Attention"),
                         QObject::tr("L'audit est terminé ! Vous pouvez le rendre modifiable en "
                                     "le dé-terminant ou en le dupliquant."));
    return false;
  }
  ...
  if ((yearDF > auditLastYear || yearDF < auditFirstYear) && (yearRF > auditLastYear || yearRF < auditFirstYear) &&
      (yearDI > auditLastYear || yearDI < auditFirstYear) && (yearRI > auditLastYear || yearRI < auditFirstYear))

  {
    QMessageBox::warning(nullptr, QObject::tr("Attention"),
                         QObject::tr("Aucune année du reporting ne se retrouve dans l'audit. "
                                     "Vous devez tout d'abord étendre les années sur lesquelles "
                                     "porte l'audit en le dupliquant.\n"
  ...
  QProgressDialog progress(QObject::tr("Mise à jour des données de l'audit..."), QObject::tr("Annuler"), 0,
                           progressMax);
...
unsigned int PCx_Reporting::duplicateReporting(const QString &newName) const {
  if (newName.isEmpty() || newName.size() > MAXOBJECTNAMELENGTH) {
    qFatal("Assertion failed, name out-of-bound");
  }
  unsigned int newReportingId = addNewReporting(newName, attachedTreeId);
  if (newReportingId == 0) {
    return 0;
  }
...
unsigned int PCx_Reporting::addNewReporting(const QString &name, unsigned int attachedTreeId) {
  ...
  if (!q.exec(QString("create table reporting_DF_%1(id integer primary key "
                      "autoincrement,"
                      " id_node integer not null references arbre_%2(id) on "
                      "delete restrict, date integer not null, "
                      "ouverts integer, realises integer, engages integer, "
                      "disponibles integer, "
                      "bp integer, reports integer, ocdm integer, vcdm "
                      "integer, budgetvote integer, "
                      "vcinterne integer, rattachenmoins1 integer, "
                      "unique(id_node,date) on conflict replace)")
                  .arg(uLastId)
                  .arg(attachedTreeId))) {
    qCritical() << q.lastError();
    die();
  ...
  if (!q.exec(QString("create index idx_idnode_reporting_DF_%1 on "
                      "reporting_DF_%1(id_node)")
                  .arg(uLastId))) {
    qCritical() << q.lastError();
    die();
  }

  if (!q.exec(QString("create table reporting_RF_%1(id integer primary key "
                      "autoincrement,"
                      " id_node integer not null references arbre_%2(id) on "
                      "delete restrict, date integer not null, "
                      "ouverts integer, realises integer, engages integer, "
  ...
  if (!q.exec(QString("create index idx_idnode_reporting_RF_%1 on "
                      "reporting_RF_%1(id_node)")
                  .arg(uLastId))) {
    qCritical() << q.lastError();
    die();
  }

  if (!q.exec(QString("create table reporting_DI_%1(id integer primary key "
                      "autoincrement, "
                      "id_node integer not null references arbre_%2(id) on "
                      "delete restrict, date integer not null, "
                      "ouverts integer, realises integer, engages integer, "
  ...
  if (!q.exec(QString("create index idx_idnode_reporting_DI_%1 on "
                      "reporting_DI_%1(id_node)")
                  .arg(uLastId))) {
    qCritical() << q.lastError();
    die();
  }

  if (!q.exec(QString("create table reporting_RI_%1(id integer primary key "
                      "autoincrement, "
                      "id_node integer not null references arbre_%2(id) on "
                      "delete restrict, date integer not null, "
                      "ouverts integer, realises integer, engages integer, "
  ...
  if (!q.exec(QString("create index idx_idnode_reporting_RI_%1 on "
                      "reporting_RI_%1(id_node)")
                  .arg(uLastId))) {
    qCritical() << q.lastError();
    die();
  }
